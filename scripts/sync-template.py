...
FILES = ("docker-compose.yml", "mcp-proxy.conf", "scripts/start.sh", "scripts/bootstrap.sh",
         ...
         "scripts/resource-sample.py", "scripts/install-observability.sh")
...
def synced_template():
    ...
    embedding = "\n".join("          printf '%s' '{}' | base64 -d | gzip -d > \"$work/{}\"".format(
        base64.b64encode(compressed((ROOT / name).read_bytes())).decode(), name) for name in FILES)
